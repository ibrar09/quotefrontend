...
export const RevenueTrendChart = ({ data }) => {
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 h-96 transition-all hover:shadow-xl group">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h3 className="text-lg font-bold text-gray-800">Revenue Trend</h3>
                    <p className="text-xs text-gray-400 font-medium mt-1">Last 6 Months Performance</p>
                </div>
...
export const StatusPieChart = ({ data }) => {
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 h-96 transition-all hover:shadow-xl">
...
export const BrandPerformanceChart = ({ data }) => {
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 h-96 transition-all hover:shadow-xl">
            <h3 className="text-lg font-bold text-gray-800 mb-6">Top Brands by Revenue</h3>
            <ResponsiveContainer width="100%" height="85%">
                <BarChart layout="vertical" data={data} barSize={24} margin={{ left: 20 }}>
                    ...
                    />
                    <Tooltip content={<CustomTooltip formatter={(val) => [`${(val / 1000).toFixed(1)}k`, 'Revenue']} />} cursor={{ fill: '#f8fafc', radius: 8 }} />
...
export const WorkTypeChart = ({ data }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 h-96 overflow-y-auto transition-all hover:shadow-xl custom-scrollbar">
            <h3 className="text-lg font-bold text-gray-800 mb-6">Revenue & Volume by Work Type</h3>
...
export const LeaderboardCard = ({ title, data, icon: Icon, color = "bg-blue-500" }) => {
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 h-96 overflow-y-auto transition-all hover:shadow-xl custom-scrollbar">
            <div className="flex items-center gap-3 mb-6 sticky top-0 bg-white z-10 pb-2 border-b border-gray-50">
            ...
            <div className="space-y-4">
                {data && data.length > 0 ? (
                    data.map((item, index) => (
                        <div key={index} className="group flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-all duration-200 border border-transparent hover:border-gray-100">
                            <div className="flex items-center gap-4">
                                <div className={`relative w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-md ${index < 3 ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-gray-200 text-gray-500'}`}>
                                    {index + 1}
                                    {index < 3 && <div className="absolute -top-1 -right-1 w-3 h-3 bg-white rounded-full border-2 border-yellow-400" />}
                                </div>
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400 font-medium">Rank #{index + 1}</p>
                                </div>
                            </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                        <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center mb-2">
                            <div className="w-6 h-6 border-b-2 border-gray-300 rounded-full"></div>
                        </div>
